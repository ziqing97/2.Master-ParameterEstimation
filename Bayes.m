...
% %Lambda
% sigma_post = inv(sigma_y^-1 * eye(length(x),length(x)) * A_select' * A_select + sigma_x^-1);
% 
% mu = sigma_y^-1 * eye(length(x),length(x)) * sigma_post * A_select' * y_select; 
...
Y_MAP = mu(1) + mu(2)*T;
Q_y_MAP = sigma_y*eye(length(Y_MAP),length(Y_MAP)) + A_T* sigma_post* A_T';
